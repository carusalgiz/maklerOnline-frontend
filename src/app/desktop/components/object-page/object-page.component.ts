...
export class ObjectPageComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnInit() {
    ...
    this.photos = [
      ...
      { 'image': '../assets/exm1.jpg' },
      { 'image': '../assets/exm2.jpg' },
      { 'image': '../assets/exm3.jpg' }
    ];
  ...
  ngAfterViewInit() {
    ...
    //   console.log(one.item(0).offsetHeight + ' ' + two.item(0).offsetHeight + ' ' + three.item(0).offsetHeight);
  ...
  ngOnChanges() {
    ...
    let item = document.getElementById('imgdesk' + this.item.id);
    console.log(item);
    if (item !== undefined && item !== null) {
      if (this.favourite === true) {
        item.style.setProperty('background-position', '50% -30px');
