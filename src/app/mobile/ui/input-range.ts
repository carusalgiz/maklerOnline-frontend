...
@Component({
    ...
    inputs: ["name", "valueMin", "valueMax", "min", "max", "step", "mode"],
    styles: [`
        :host{
            position: relative;
            height: 45px;
    ...
    `],
    template: `
        <div [class.desktop]="mode == 'desktop'"><span class="label">{{name}}</span><span>{{valueMin > 0 ? "от " + valueMin + " 000 Р" : ""}} {{valueMax < max ? "до " + valueMax  + " 000 Р" : ""}}</span></div>
        <input [class.desktop]="mode == 'desktop'" [min]="min" [max]="max" [step]="step" type="range" [(ngModel)]="valueMin" (ngModelChange)="change()" (change)="emitChange()">
        <input [class.desktop]="mode == 'desktop'" [min]="min" [max]="max" [step]="step" type="range" [(ngModel)]="valueMax" (ngModelChange)="change()" (change)="emitChange()"
